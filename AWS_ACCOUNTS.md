...
Cluo migrated off homelab's shared VPS to its own dedicated Hetzner VPS
(2026-06); homelab's Terraform-managed `cluo-minio-backups-staging` bucket
and its `cluo-minio-backup-staging-user` IAM user were decommissioned along
with that move — cluo's backups now go through its own Terraform/AWS
resources instead.
