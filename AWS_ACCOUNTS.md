...
| `cluo-minio-backup-staging-user` | Least-privilege credential for the `cluo-staging-backup` container (homelab `docker/cluo/staging.docker-compose.yml`), which uploads GPG-encrypted MinIO snapshots. Provisioned by **homelab's** Terraform (`homelab/terraform/main.tf`), not cluo's — cluo's own Terraform is an unprovisioned future-deployment blueprint, not what's actually live on this server. | `cluo-minio-backups-staging` bucket only — Get/Put/Delete/List, no other permissions. | not in `~/.aws/credentials` — used directly inside the backup container, not from a developer's CLI. |
...
provisioned by homelab's Terraform), `homelab-backups-henga` (nightly
Vaultwarden backup).
